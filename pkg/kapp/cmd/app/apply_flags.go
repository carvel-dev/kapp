...
func (s *ApplyFlags) SetWithDefaults(prefix string, defaults ApplyFlags, cmd *cobra.Command) {
	...
	cmd.Flags().DurationVar(&s.WaitingChangesOpts.ResourceWaitTimeout, prefix+"wait-resource-timeout",
		mustParseDuration("15m"), "Maximum amount of time to wait for each resource in wait phase")
